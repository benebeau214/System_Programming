...
struct bitmap *bitmap_expand(struct bitmap *b, int size) {
  if (b == NULL) {
      return NULL; // 기존 비트맵이 NULL이면 확장할 수 없음.
  }

  size_t new_size = b->bit_cnt + size; // 기존 크기 + 추가 크기

  struct bitmap *new_b = bitmap_create(new_size);
  if (new_b == NULL || new_b->bits == NULL) {
      return NULL; // 메모리 할당 실패 시 NULL 반환
  }

  // 기존 데이터를 새로운 비트맵으로 복사
  size_t old_bytes = byte_cnt(b->bit_cnt); // 기존 비트맵 크기
  size_t new_bytes = byte_cnt(new_size); // 새 비트맵 크기

  if (b->bits != NULL) {
      memcpy(new_b->bits, b->bits, old_bytes);  // 기존 데이터 복사
  }

  memset((uint8_t *)new_b->bits + old_bytes, 0, new_bytes - old_bytes); // 새 영역 0으로 초기화

  bitmap_destroy(b); // 기존 비트맵 삭제
  return new_b;
}
